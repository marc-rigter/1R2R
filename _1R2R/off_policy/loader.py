...
def restore_pool(
    ...
    if 'datasets' in experiment_root:
        assert dataset_dir is not None
        data = restore_dataset(experiment_root[9:], dataset_dir)

    if 'mdp_examples' in experiment_root:
        data = gen_dataset(experiment_root[13:])
...
def restore_dataset(name, dataset_dir):
