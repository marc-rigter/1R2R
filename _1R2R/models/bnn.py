...
class BNN:
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        if not self.separate_mean_var:
            ...
            with self.sess.as_default():
                with tf.variable_scope(self.name):
                    ...
                    self.max_logvar = tf.Variable(np.ones([1, self.layers[-1].get_output_dim() // 2]), dtype=tf.float32,
                                                  name="max_log_var")
                    self.min_logvar = tf.Variable(-np.ones([1, self.layers[-1].get_output_dim() // 2])*6., dtype=tf.float32,
                                                  name="min_log_var")
            ...
            with self.sess.as_default():
                with tf.variable_scope(self.name):
                    self.scaler = TensorStandardScaler(self.layers[0].get_input_dim())
                    self.max_logvar = tf.Variable(np.ones([1, self.var_layers[-1].get_output_dim()]), dtype=tf.float32,
                                                  name="max_log_var")
                    self.min_logvar = tf.Variable(-np.ones([1, self.var_layers[-1].get_output_dim()])*6., dtype=tf.float32,
                                                  name="min_log_var")
                    for i, layer in enumerate(self.layers):
        ...
        with tf.variable_scope(self.name):
            ...
            if not self.deterministic:
                ...
                train_loss += 0.001 * tf.reduce_sum(self.max_logvar) - 0.001 * tf.reduce_sum(self.min_logvar)
            else:
    ...
    def validate(self):
        assert (self.holdout_inputs is not None) and (self.holdout_targets is not None) \
            , "Must run train() before this function."
        losses = self.sess.run(
            self.mse_loss,
            feed_dict={
                self.sy_train_in: self.holdout_inputs,
                self.sy_train_targ: self.holdout_targets
                }
        )
        losses = np.sort(losses)[:self.num_elites]
        return losses
    ...
    def train(self, inputs, targets,
        ...
        self.holdout_inputs = holdout_inputs
        ...
        elite_losses = (np.sort(holdout_losses)[:self.num_elites])
        model_metrics = {'supervised_val_loss_avg': elite_losses.mean()}
        for i in range(len(elite_losses)):
            model_metrics.update({'supervised_val_loss_' + str(i): elite_losses[i]})
    ...
    def get_labeled_batch(self, batch_size=256):
        """ Get a batch of labeled data from the training dataset

        Returns:
            inputs: tensor of inputs from real dataset.
            targets: corresponding tensor of targets from real dataset
        """

        assert (self.inputs is not None) and (self.targets is not None)
        idxs = np.random.randint(self.inputs.shape[0], size=[self.num_nets, batch_size])
