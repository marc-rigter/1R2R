...
def construct_model(obs_dim=11, act_dim=3, rew_dim=1, hidden_dim=200, num_networks=7,
					...
					name=None, load_dir=None, deterministic=False, weight_decay=None):
	if name is None:
		name = 'BNN'
	...
	if not model.model_loaded:
		if model_type == 'identity':
			...
			if weight_decay is None:
				...
				print('[ BNN ] Using weight decay: {}'.format(weight_decay))
				model.add(FC(hidden_dim, input_dim=obs_dim+act_dim, activation="swish", weight_decay=weight_decay))
				model.add(FC(hidden_dim, activation="swish", weight_decay=weight_decay))
				model.add(FC(hidden_dim, activation="swish", weight_decay=weight_decay))
				model.add(FC(hidden_dim, activation="swish", weight_decay=weight_decay))
				model.add(FC(obs_dim+rew_dim, weight_decay=weight_decay))
				if separate_mean_var:
					model.add(FC(obs_dim+rew_dim, input_dim=hidden_dim, weight_decay=weight_decay), var_layer=True)

	if load_dir is not None:
		model.model_loaded = True

	model.finalize(tf.train.AdamOptimizer, {"learning_rate": 3e-4})
