...
class RLAlgorithm(tf.contrib.checkpoint.Checkpointable):
    ...
    def __init__(
        ...
        self._cvar_levels = [0.05, 0.1, 0.2, 0.25, 0.3, 0.5]
        self._wang_levels = [0.5, 0.75, 1.0, 1.5, 2.0, 3.0]
        self._avg_cvars = {str(cvar): [] for cvar in self._cvar_levels}
        self._avg_wangs = {str(wang): [] for wang in self._wang_levels}

        self._avg_returns = list()
        self._avg_returns_num_iter = 10
        ...
        print('WANDB Group: {}'.format(wandb_group))
    ...
    def _train(self):
        ...
        for self._epoch in gt.timed_for(range(self._epoch, self._n_epochs)):
            ...
            if self._epoch % self._evaluate_interval == 0 \
                or self._epoch >= self._n_epochs - 10:
    ...
    def _evaluation_paths(self, policy, evaluation_env, obs_mean=None, obs_std=None):
    ...
    def _evaluate_rollouts(self, paths, env):
        ...
        self._avg_returns.insert(0, np.mean(total_returns))
        self._avg_returns = self._avg_returns[0:self._avg_returns_num_iter]
        avg_recent_iter = np.mean(np.array(self._avg_returns))

        diagnostics = OrderedDict((
            ('return-average', np.mean(total_returns)),
            ('return-average-last-' + str(self._avg_returns_num_iter) + '-iter', \
            avg_recent_iter),
            ...
            ('full-horizon', episode_lengths.count(self.sampler._max_path_length) / len(episode_lengths))
        ))

        # distance for antmaze
        last_infos = [path['infos'][-1] for path in paths]
        if 'end_dist' in last_infos[0]:
            dists = np.array([info['end_dist'] for info in last_infos])
            vec_to_goal = np.array([0.0, 0.0])
            for i in range(len(last_infos)):
                vec_to_goal += last_infos[i]['vec_to_goal']
            avg_vec_to_goal = vec_to_goal / len(last_infos)
            diagnostics.update({
                'avg-goal-dist': np.mean(dists),
                'std-goal-dist': np.std(dists),
                'min-goal-dist': np.min(dists),
                'max-goal-dist': np.max(dists),
                'avg-vec-to-goal-x': avg_vec_to_goal[0],
                'avg-vec-to-goal-y': avg_vec_to_goal[1]
            })

        returns_sorted = sorted(total_returns)
        for level in self._cvar_levels:
            cvar_vals = returns_sorted[:int(np.round(level * len(total_returns)))]
            cvar = sum(cvar_vals) / len(cvar_vals)
            diagnostics.update({'return-cvar-' + str(level): cvar})

            self._avg_cvars[str(level)].insert(0, np.mean(cvar))
            avg_recent_iter = np.mean(self._avg_cvars[str(level)][0:self._avg_returns_num_iter])
            diagnostics.update({'avg-cvar-' + str(level) + '-last-' + str(self._avg_returns_num_iter) + '-iter': avg_recent_iter})


        for level in self._wang_levels:
            wang = eval_wang_risk(total_returns, level)
            #diagnostics.update({'wang-' + str(level): wang})

            self._avg_wangs[str(level)].insert(0, wang)
            avg_recent_iter = np.mean(self._avg_wangs[str(level)][0:self._avg_returns_num_iter])
            diagnostics.update({'wang-' + str(level) + '-avg-last-' + str(self._avg_returns_num_iter) + '-iter': avg_recent_iter})
